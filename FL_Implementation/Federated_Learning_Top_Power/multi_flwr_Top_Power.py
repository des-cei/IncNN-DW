import subprocess
import os
from test_functions import preprocessing_top_power, model_testing, replace_strategy_in_file, modify_proximal_mu, modify_k_fold, prepare_test_database
import pickle
import numpy as np
import pandas as pd
...
# Create dummy dictionary for getting iteration results and then compute mean and sdv
...
model_type = "Top-power"
model = preprocessing_top_power()

# Variables for running flwr command
cwd = os.getcwd()
working_directory = cwd + "/Federated_Learning_Top_Power"
command = ["flwr", "run"]

# Variables to modify strategy for flwr
strategy_file = cwd + "/Federated_Learning_Top_Power/Program/server_app.py"
strategy_target_line = 48
FedProx_target_line = 54
k_fold_file = cwd + "/Federated_Learning_Top_Power/Program/task.py"
