...
with open('Resultados_FL_sr1_server_outlier_free.pkl', 'rb') as handle:
...
for model in model_type:
    ...
    for rt in result_type:
        ...
        for mode in range(len(name)/2):
            ...
            plt.savefig("Pareto_no_outliers_" + model + "_" + rt + "_sr1_" + name[2*mode] + "_vs_" + name[2*mode+1] + ".tiff")
            plt.show()
        print("\n")
exit()
